...
class BookService:
    ...
    async def add_book(self, user_id: int, title: str) -> Book:
        return await self._repo.create_book(user_id, title)


    async def mark_pages(self, user_id: int, book_id: int, pages: int) -> Book:
...
class StatsService:
    ...
    async def get_stats(self, user_id: int) -> UserStats:
        total_books = await self.repo.fetch_books(user_id)
        total_pages = await self.repo.update_pages(user_id)
        return UserStats(user_id, total_books, total_pages)